...
def _envelope_follower(signal: np.ndarray, sr: int,
    ...
    """Vectorized envelope follower using scipy lfilter (~50-100x faster)."""
    avg_ms = (attack_ms + release_ms) / 2.0
    coeff = np.exp(-1.0 / (avg_ms * sr / 1000.0))
    ...
    def _upward_compress(data, sr, amount_db, threshold_db=-40.0):
        ...
        for ch in range(data.shape[1] if data.ndim > 1 else 1):
            ...
            gain_db = np.where(
                ...
                np.where(env_db < -10,
                         amount_db * (1.0 - (env_db - threshold_db) / (-10 - threshold_db)),
                         0.0))
...
class MasterChain:
    ...
    def load_audio(self, path: str) -> bool:
        ...
        self.output_path = f"{base}_mastered{ext}"
        self.preview_path = f"{base}_preview{ext}"
        return True
    ...
    def _send_meter(self, data: np.ndarray, sr: int, stage: str):
        """Send real-time meter data to GUI.
    ...
    def preview(
        ...
        try:
            ...
            preview_path = self.preview_path or "/tmp/longplay_preview.wav"
    ...
    def _preview_real(self, preview_path, start_sec, duration_sec, callback):
        ...
        preview_data = data[start_sample:end_sample]
